...
public class JavaTypeNameTranslator {
	 ...
	 * .ColumnDefinition)
	 */
	public static String getJavaTypeName(final ResultSetMetaData resultSetMetaData, final int column)
			throws SQLException {
		...
		case Types.DECIMAL:
			if ((resultSetMetaData.getScale(column) > 0) || (resultSetMetaData.getColumnDisplaySize(column) > 18)
					|| forceBigDecimals) {
				answer = BigDecimal.class.getName();
			} else if (resultSetMetaData.getColumnDisplaySize(column) > 9) {
				answer = Long.class.getName();
			} else if (resultSetMetaData.getColumnDisplaySize(column) > 4) {
		...
		case Types.NUMERIC:
			if ((resultSetMetaData.getScale(column) > 0) || (resultSetMetaData.getPrecision(column) > 18)
					|| forceBigDecimals) {
				answer = BigDecimal.class.getName();
			} else if (resultSetMetaData.getPrecision(column) > 9) {
				answer = Long.class.getName();
			} else if (resultSetMetaData.getPrecision(column) > 4) {
				answer = Integer.class.getName();
			} else {
				// answer = Short.class.getName();
				answer = Integer.class.getName(); // XingXiuDong add for javaType of Domain
